...
final class SourceEditorAdapterTests: XCTestCase {
    ...
    func testCodeMirrorSelectionCommandsAndReadOnlyGuardAreRoutedDirectly() {
        ...
        XCTAssertTrue(html.contains("function performTextTransform(view, transform)"), html)
        XCTAssertTrue(html.contains("function duplicateSelectionOrCurrentLine(view)"), html)
        XCTAssertTrue(html.contains("function transformLineText(text, transform)"), html)
        XCTAssertTrue(html.contains("function performMarkdownCommand(view, command)"), html)
        XCTAssertTrue(html.contains("function wrapMarkdownSelections(view, left, right)"), html)
        XCTAssertTrue(html.contains("function transformMarkdownLines(view, transform)"), html)
        XCTAssertTrue(html.contains("function insertMarkdownCodeFence(view)"), html)
        ...
        XCTAssertTrue(html.contains("if (command.startsWith(\"transform:\")) return performTextTransform(editorView, command.slice(\"transform:\".length));"), html)
        XCTAssertTrue(html.contains("if (command.startsWith(\"markdown:\")) return performMarkdownCommand(editorView, command.slice(\"markdown:\".length));"), html)
