...
- After the editor hot-path stabilization change, the focused suite passed:
  ...
  Swift fallback path.
  The CodeMirror direct Markdown command follow-up passed
  `SourceEditorAdapterTests` for 14 tests, embedded-JS syntax validation, and
  the full suite for 640 tests with 1 skipped and 0 failures; the full-suite
  headless probes measured 16.53 ms for 25k-line text indexing, 0.68 ms for
  visible line reads, 156.45 ms for 16k/27-column CSV indexing, 1.68 ms for
  visible row reads, and 25.63 ms for 240 visible JSON line highlights.
  `git diff --check` also passed. Common inline Markdown wrappers,
  heading/list/task/quote transforms, links, image/table snippets, code fences,
  math blocks, and Mermaid snippets now route directly through the CodeMirror
  WebView bridge. JSON minify/format and Markdown table formatting
  intentionally remain on the Swift fallback path.
  The Activity
...
- After the CodeMirror direct selection/line movement/comment command and read-only guard slice, the same
  ...
  `Scratch 1`.
- After the CodeMirror direct Markdown command slice, the same safe-mode verify
  command rebuilt the app, relaunched `dist/SimpleLime.app` without
  screenshots, confirmed `/health` on the local automation bridge at port
  48777, and left a fresh safe-mode `SimpleLime` process running as PID 79630
  on `Scratch 1`.
